...
3. `just ml-label` opens a local keyboard-driven page that plays each unlabeled unit and preselects the model's guessed label. Press `b`/`m`/`s`/`n`/`o` to assign `bark`/`meow`/`speech`/`noise`/`other`, `Enter` to accept the highlighted suggestion, `Space` to replay, and `u` to undo; each keystroke writes `labels.csv` atomically, so a crash loses nothing. On first run it computes AST suggestions for every remaining unit, filling the same embedding cache `ml-embed` uses (pass `--no-suggest` to skip the model, `--no-browser`/`--port` to control the server). Hand-editing `scripts/ml_prototype/data/labels.csv` still works: fill the `label` column of raw-derived rows with one of `bark`, `meow`, `speech`, `noise`, or `other`. Library rows come prefilled, and re-running `ml-prep` preserves your labels while appending newly discovered units.
