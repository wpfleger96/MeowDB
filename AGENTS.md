...
**Audio pipeline classifier profiles** -- `SegmentationConfig.classifier` selects `"feline"` (cat, default) or `"canine"` (dog); per-species overrides live in `SPECIES_REGISTRY` (`species.py`), where the dog's fingerprint band (60–3500 Hz) intentionally differs from its detection band (150–3500 Hz). The canine classifier first splits each VAD candidate into per-sub-unit classification units at interior quiet runs (else a bark fuses with adjacent speech and reads as speech), then accepts a unit on Gate G (in-band dominance) AND any of three branches: sharp bark (voiced, not broadband — real barks measure low spectral flatness, impacts are the broadband sounds — weakly voiced, F0 ≥ 300 Hz, fast attack), bark bout / howl (sustained, strongly voiced, F0 ≥ 340 Hz), or sustained boof (long, voiced, less harmonic than speech, F0 150–250 Hz, low-band-dominant spectrum). Canine-only knobs live in the nested `SegmentationConfig.canine` (`CanineConfig`), not as flat fields.
