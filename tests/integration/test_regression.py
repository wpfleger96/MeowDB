...
Covers:
  ...
  3. Ingest batch resilience: sound with missing MP3 kept local; rest of batch migrates
...
def _sound_row(
    wav_path: str = "audio/wav/x.wav",
    mp3_path: str = "audio/mp3/x.mp3",
    *,
    animal_id: str,
) -> dict:
    return {
...
def test_suffix_range_returns_last_n_bytes(s3_api_client):
    ...
    sound_id = app.state.db.add(_sound_row(animal_id=animal_id))
    s3.put_object(Bucket=_BUCKET, Key=mp3_key(sound_id), Body=content)

    resp = tc.get(f"/api/audio/{sound_id}", headers={"Range": "bytes=-50"})
...
def test_suffix_range_zero_returns_416(s3_api_client):
    ...
    animal_id = app.state.db.get_animals()[0]["id"]
    sound_id = app.state.db.add(_sound_row(animal_id=animal_id))
    s3.put_object(Bucket=_BUCKET, Key=mp3_key(sound_id), Body=bytes(range(100)))

    resp = tc.get(f"/api/audio/{sound_id}", headers={"Range": "bytes=-0"})
...
def test_suffix_range_larger_than_file_returns_whole_file(s3_api_client):
    ...
    resp = tc.get(f"/api/audio/{sound_id}", headers={"Range": "bytes=-200"})
...
def test_audio_serve_absolute_path_in_s3_mode_serves_from_disk(s3_api_client, tmp_path):
    """A sound row with an absolute local MP3 path is served from disk (not 404) in S3 mode."""
    ...
    animal_id = app.state.db.get_animals()[0]["id"]
    sound_id = app.state.db.add(
        _sound_row(
            wav_path=str(tmp_path / "wav" / "x.wav"),
...
@pytest.mark.integration
def test_sound_delete_absolute_path_in_s3_mode_unlinks_local_files(s3_api_client, tmp_path):
    """Deleting a sound with absolute local paths in S3 mode removes the local files."""
    tc, app, _s3 = s3_api_client
    ...
    animal_id = app.state.db.get_animals()[0]["id"]
    sound_id = app.state.db.add(
        _sound_row(wav_path=str(wav_file), mp3_path=str(mp3_file), animal_id=animal_id)
    )

    resp = tc.delete(f"/api/sounds/{sound_id}")
...
def test_photo_serve_uses_local_file_when_present_in_s3_mode(s3_api_client, tmp_path):
    ...
    animal_id = app.state.db.get_animals()[0]["id"]
    app.state.db.add_photo(filename, animal_id)
...
def test_photo_delete_removes_local_file_and_s3_object(s3_api_client, tmp_path):
    ...
    resp = tc.delete(f"/api/animals/{animal_id}/photos/{photo_id}")
...
# ===========================================================================
# 3. Ingest batch resilience: missing MP3 → sound keeps local paths; rest migrates
# ===========================================================================


@pytest.mark.integration
def test_upload_committed_to_s3_skips_sound_with_missing_mp3(s3_state, tmp_path):
    """_upload_committed_to_s3 skips a sound whose local MP3 is absent.

    The skipped sound retains its original DB paths; the complete sound migrates.
    """
    wav_dir = tmp_path / "wav"
    mp3_dir = tmp_path / "mp3"
    wav_dir.mkdir()
    ...
    sound1_id = db.add(_sound_row(animal_id=animal_id))
    sound2_id = db.add(_sound_row(animal_id=animal_id))

    # sound1 has both WAV and MP3 local files
    (wav_dir / f"{sound1_id}.wav").write_bytes(b"wav-1")
    (mp3_dir / f"{sound1_id}.mp3").write_bytes(b"mp3-1")

    # sound2 has only WAV — MP3 is missing
    (wav_dir / f"{sound2_id}.wav").write_bytes(b"wav-2")

    with (
        patch("meowdb.api.routers.ingest.WAV_DIR", wav_dir),
        patch("meowdb.api.routers.ingest.MP3_DIR", mp3_dir),
    ):
        asyncio.run(_upload_committed_to_s3(db, [sound1_id, sound2_id]))

    bucket_keys = [o["Key"] for o in s3_state.list_objects_v2(Bucket=_BUCKET).get("Contents", [])]

    # sound1 fully migrated
    assert f"audio/wav/{sound1_id}.wav" in bucket_keys
    assert f"audio/mp3/{sound1_id}.mp3" in bucket_keys
    m1 = db.get_by_id(sound1_id)
    assert m1 is not None
    assert m1["wav_path"] == f"audio/wav/{sound1_id}.wav"
    assert not (wav_dir / f"{sound1_id}.wav").exists()

    # sound2 skipped — no S3 objects, local WAV still present, DB path NOT changed
    assert f"audio/wav/{sound2_id}.wav" not in bucket_keys
    m2 = db.get_by_id(sound2_id)
    assert m2 is not None
    assert m2["wav_path"] == "audio/wav/x.wav"  # unchanged from initial seed value
    assert (wav_dir / f"{sound2_id}.wav").exists()
...
def test_photo_upload_s3_failure_leaves_no_db_record(s3_api_client):
    ...
    animal_id = app.state.db.get_animals()[0]["id"]
    with patch(
        "meowdb.api.routers.animals.upload_to_s3", side_effect=RuntimeError("S3 unavailable")
    ):
...
def test_edit_photo_s3_in_place_webp(s3_api_client, tmp_path):
    ...
    animal_id = app.state.db.get_animals()[0]["id"]
    app.state.db.add_photo(filename, animal_id, photo_id=photo_id)
...
def test_edit_photo_s3_format_converting_jpg_to_webp(s3_api_client, tmp_path):
    ...
    animal_id = app.state.db.get_animals()[0]["id"]
    app.state.db.add_photo(jpg_filename, animal_id, photo_id=photo_id)
