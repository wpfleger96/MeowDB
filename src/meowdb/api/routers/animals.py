...
async def delete_animal(
    ...
    for audio_path in result.get("audio_paths", []):
        if is_s3_enabled() and is_s3_key(audio_path):
            try:
                await delete_from_s3(audio_path)
            except Exception:
                _logger.warning("Failed to remove S3 audio object %s", audio_path)
            continue
        p = Path(audio_path)
    ...
    for photo_filename in result.get("photo_filenames", []):
        ...
        if is_s3_enabled():
            try:
                await delete_from_s3(photo_key(photo_filename))
            except Exception:
                _logger.warning("Failed to remove S3 photo object %s", photo_filename)
